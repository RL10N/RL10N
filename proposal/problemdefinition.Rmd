...
n_pkgs_with_trans <- n_trans_data %>% 
  filter_(~ x > 0) %>% 
  summarise_(~ sum(freq)) %>% 
  as.integer

n_pkgs_with_1_trans <- n_trans_data %>% 
  filter_(~ x == 1) %>% 
  summarise_(~ sum(freq)) %>% 
  as.integer  
```

R has been translated into [20 languages](http://developer.r-project.org/TranslationTeams.html) but currently not many packages have translations. Indeed of the 8274 packages on CRAN (actually, as mirrored on GitHub from CRAN), only `r n_pkgs_with_trans` have any installed translations. Of those, `r n_pkgs_with_1_trans` have only a single translation.  As Figure 1 shows, the number of translated packages is almost indistinguishable from zero.


```{r, NumberOfTranslationsPerPkg, echo = FALSE, fig.width = 9, fig.height = 6, fig.cap = "Figure 1: Number of translations per R package."}
...
Further, the selection of languages that are translated to is relatively limited.  As Figure 2 shows ... **TODO: Once we have R User Group numbers (or other usage-of-r stats), pick out any languages with users but no translations.**

```{r, NumberOfTranslationsByLang, echo = FALSE, fig.width = 9, fig.height = 6, fig.cap = "Figure 2: Number of translations by language."}
...
Beyond this, finding multi-lingual R programmers with enough free time to undertake translation duties is even harder. 
